...
const getRandomColor: () => string = () => {
  ...
  const isWhite = color.slice(1).split('').filter((_, i) => i % 2 === 0).every(i => ['f', 'e', 'd', 'c'].includes(i))
...
const LabelCard: React.FC = () => {
  ...
  return <div className='label-card'>
    <Input
      className='input'
      suffix={
        <SearchOutlined style={{ color: '#d4d4d4' }} />
      }
      placeholder='搜索已有标签'
      onChange={res =>
    ...
    <div className='tag-wrapper'>
      {
        labelData.filter(i => i.includes(searchText)).map(i => <Tag className='tag' closable color={getRandomColor()} key={i}>{i}</Tag>)
      }
    </div>
  </div>
}
...
const HotCaseCard: React.FC = () => {
  ...
  return <div className='hot-case-card'>
    <Input
      className='input'
      suffix={
        <SearchOutlined style={{ color: '#d4d4d4' }} />
    ...
    <div className='list'>
      {hotCaseDataState.filter(i => i.title.includes(searchText)).map((item, index) =>
...
const CasesCard: React.FC = () => {
  ...
  return <div className='cases-card'>
    <Radio.Group className='radio-group' defaultValue={currentTab.key} size='small' buttonStyle='solid' onChange={res => {
      setCurrentTab(tabs.find(i => i.key === res.target.value) || tabs[0])
    }}>
      {tabs.map(i => (
        <Radio.Button key={i.key} className='radio-btn' value={i.key}>{i.label}</Radio.Button>
      ))}
    </Radio.Group>

    <TabCard title={currentTab.label} content={renderContent[currentTab.key]} />
  </div>
}
